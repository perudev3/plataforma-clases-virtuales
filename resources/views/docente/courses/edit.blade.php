...
<style>
  ...
  .media-current { display: flex; align-items: center; gap: 8px; margin-top: 8px; padding: 8px 10px; background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 8px; }
  .media-current-thumb { width: 40px; height: 40px; border-radius: 6px; object-fit: cover; flex-shrink: 0; }
  .media-current-thumb-placeholder { width: 40px; height: 40px; border-radius: 6px; background: #e2e8f0; display: flex; align-items: center; justify-content: center; flex-shrink: 0; }
  .media-current-thumb-placeholder svg { width: 18px; height: 18px; fill: none; stroke: #94a3b8; stroke-width: 1.5; stroke-linecap: round; stroke-linejoin: round; }
  .media-current-label { font-family: 'Montserrat', sans-serif; font-size: 11px; font-weight: 600; color: #475569; }
  .media-current-name { font-size: 11px; color: #94a3b8; margin: 1px 0 0; }
  ...
  /* Edit badge */
  .edit-badge { display: inline-flex; align-items: center; gap: 5px; background: rgba(201,162,77,.12); color: #9a7530; border: 1px solid rgba(201,162,77,.3); border-radius: 20px; padding: 3px 10px; font-family: 'Montserrat', sans-serif; font-size: 11px; font-weight: 700; }
  .edit-badge svg { width: 11px; height: 11px; fill: none; stroke: currentColor; stroke-width: 2.5; stroke-linecap: round; stroke-linejoin: round; }
  ...
  <div class="form-page-header">
    <div style="display:flex;flex-direction:column;gap:6px;">
      <div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;">
        <h1 class="form-page-title">Editar curso</h1>
        <span class="edit-badge">
          <svg viewBox="0 0 24 24"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
          Editando
        </span>
      </div>
      <p class="form-page-sub">{{ $course->title }}</p>
    </div>
      ...
      <div class="form-section-body">
        <div class="f-grid f-grid-8-4" style="margin-bottom:14px;">
          <div>
            ...
            <input type="text" name="title" class="f-control f-control-lg" placeholder="Ej: Diplomado en Gestión Pública" required value="{{ old('title', $course->title) }}">
          </div>
          <div>
            <label class="f-label">Programa *</label>
            <select name="programa" class="f-control" required>
              <option value="">Seleccionar</option>
              <option value="diplomado"       {{ old('programa', $course->programa)=='diplomado'?'selected':'' }}>Diplomado</option>
              <option value="especializacion" {{ old('programa', $course->programa)=='especializacion'?'selected':'' }}>Especialización</option>
              <option value="curso"           {{ old('programa', $course->programa)=='curso'?'selected':'' }}>Curso</option>
              <option value="seminario"       {{ old('programa', $course->programa)=='seminario'?'selected':'' }}>Seminario</option>
            </select>
          </div>
        </div>
        ...
        <div style="margin-bottom:14px;">
          ...
          <input type="text" name="subtitle" class="f-control" value="{{ old('title', $course->title) }}" maxlength="160">
        ...
        <div style="margin-bottom:14px;">
          ...
          <textarea name="description" id="editor-description" class="f-control">{{ old('description', $course->description) }}</textarea>
        </div>
        <div style="margin-bottom:14px;">
          <label class="f-label">Dirigido a</label>
          <textarea name="directed_to" id="editor-directed" class="f-control" placeholder="Profesionales, egresados, estudiantes...">{{ old('directed_to', $course->directed_to) }}</textarea>
        </div>
        <div style="max-width:240px;">
          <label class="f-label">¿Curso destacado?</label>
          <div class="toggle-group">
            <input type="radio" name="is_featured" id="feat_no"  value="0" {{ old('is_featured', $course->is_featured ? '1' : '0')=='0' ? 'checked' : '' }}>
            <label for="feat_no">No destacado</label>
            <input type="radio" name="is_featured" id="feat_yes" value="1" {{ old('is_featured', $course->is_featured ? '1' : '0')=='1' ? 'checked' : '' }}>
    ...
    <div class="form-section">
      ...
      <div class="form-section-body">
        ...
        <div class="teacher-list" id="teacherList">
          @php
            $assignedTeachers = $course->teachers->keyBy('id');
          @endphp
          @forelse($docentes as $docente)
          @php
            $isAssigned = $assignedTeachers->has($docente->id);
            $assignedRole = $isAssigned ? ($assignedTeachers[$docente->id]->pivot->role ?? 'colaborador') : 'colaborador';
          @endphp
          <label class="teacher-item {{ $isAssigned ? 'selected' : '' }}" id="item_{{ $docente->id }}">
            <input type="checkbox"
                   name="teacher_ids[]"
                   value="{{ $docente->id }}"
                   {{ $isAssigned ? 'checked' : '' }}
            ...
            <div class="teacher-role-select">
              <select name="teacher_role_{{ $docente->id }}" onclick="event.stopPropagation()">
                <option value="principal"    {{ $assignedRole=='principal'    ? 'selected' : '' }}>Principal</option>
                <option value="colaborador"  {{ $assignedRole=='colaborador'  ? 'selected' : '' }}>Colaborador</option>
                <option value="invitado"     {{ $assignedRole=='invitado'     ? 'selected' : '' }}>Invitado</option>
              </select>
            </div>
          </label>
          @empty
      ...
      <div class="form-section-body">
        <div class="f-grid f-grid-4">
          <div>
            ...
            <input type="date" name="start_date" class="f-control" value="{{ old('start_date', $course->start_date) }}">
          </div>
          <div>
            <label class="f-label">Duración (semanas)</label>
            <input type="number" name="duration_weeks" class="f-control" placeholder="0" value="{{ old('duration_weeks', $course->duration_weeks) }}">
          </div>
          <div>
            <label class="f-label">Horas académicas</label>
            <input type="number" name="hours" class="f-control" placeholder="0" value="{{ old('hours', $course->hours) }}">
          </div>
          <div>
            <label class="f-label">Modalidad</label>
            <select name="modality" class="f-control">
              <option value="">Seleccionar</option>
              <option {{ old('modality', $course->modality)=='Virtual en vivo' ?'selected':'' }}>Virtual en vivo</option>
              <option {{ old('modality', $course->modality)=='Virtual grabado' ?'selected':'' }}>Virtual grabado</option>
              <option {{ old('modality', $course->modality)=='Presencial'      ?'selected':'' }}>Presencial</option>
              <option {{ old('modality', $course->modality)=='Mixto'           ?'selected':'' }}>Mixto</option>
            </select>
          </div>
        </div>
      </div>
      ...
      <div class="form-section-body">
        <div class="f-grid f-grid-3">
          <div>
            ...
            <input type="text" name="class_days" class="f-control" placeholder="Ej: Lunes y Miércoles" value="{{ old('class_days', $course->class_days) }}">
          </div>
          <div>
            <label class="f-label">Hora de inicio</label>
            <input type="time" name="start_time" class="f-control" value="{{ old('start_time', $course->start_time) }}">
          </div>
          <div>
            <label class="f-label">Hora de fin</label>
            <input type="time" name="end_time" class="f-control" value="{{ old('end_time', $course->end_time) }}">
          </div>
        </div>
      </div>
    </div>
...
<div class="form-section">
  ...
  <div class="form-section-body">
    ...
    <div style="margin-bottom:16px;max-width:280px;">
      ...
      <div class="toggle-group">
        <input type="radio" name="is_paid" id="free" value="0"
          {{ old('is_paid', $course->is_paid ? '1' : '0')=='0'?'checked':'' }}
          onchange="togglePrice(this.value)">
        <label for="free">Gratuito</label>

        <input type="radio" name="is_paid" id="paid" value="1"
          {{ old('is_paid', $course->is_paid ? '1' : '0')=='1'?'checked':'' }}
          onchange="togglePrice(this.value)">
    ...
    <div id="priceFields" style="display:none;">
      ...
      <div class="f-grid f-grid-3" style="margin-bottom:16px;">
        ...
        <div>
          ...
          <select name="currency" id="currencySelect" class="f-control"
            onchange="updateCurrencySymbols()">
            <option value="PEN" {{ old('currency', $course->currency ?? 'PEN')=='PEN'?'selected':'' }}>Soles (PEN)</option>
            <option value="USD" {{ old('currency', $course->currency)=='USD'?'selected':'' }}>Dólares (USD)</option>
          </select>
        </div>
      </div>
      ...
      <div style="border-top:1px solid #f1f5f9;padding-top:16px;">
        ...
        <div class="toggle-group" style="max-width:340px;">
          <input type="radio" name="price_display" id="disp_regular" value="regular"
            {{ old('price_display', $course->price_display ?? 'regular')=='regular'?'checked':'' }}
            onchange="updatePricePreview()">
          <label for="disp_regular">Precio regular</label>

          <input type="radio" name="price_display" id="disp_discount" value="discount"
            {{ old('price_display', $course->price_display)=='discount'?'checked':'' }}
            onchange="updatePricePreview()">
          <label for="disp_discount">Precio oferta</label>
        </div>

        {{-- Preview (igual que create, NO TOCAR) --}}
        <div class="price-preview-box" id="pricePreviewBox">
          ...
          <div>
            ...
            <p class="price-preview-value" id="previewBigPrice">
              <span class="currency-sym" id="previewCurrencySym">S/</span>
      ...
      <div class="form-section-body">
        <div class="f-grid f-grid-3">
          <div>
            ...
            <div class="toggle-group">
              <input type="radio" name="has_certificate" id="cert_no"  value="0" {{ old('has_certificate', $course->has_certificate ? '1' : '0')=='0' ? 'checked' : '' }}>
              <label for="cert_no">No</label>
              <input type="radio" name="has_certificate" id="cert_yes" value="1" {{ old('has_certificate', $course->has_certificate ? '1' : '0')=='1' ? 'checked' : '' }}>
          ...
          <div>
            ...
            <select name="certificate_type" class="f-control">
              <option value="">Seleccionar</option>
              <option {{ old('certificate_type', $course->certificate_type)=='Digital' ?'selected':'' }}>Digital</option>
              <option {{ old('certificate_type', $course->certificate_type)=='Físico'  ?'selected':'' }}>Físico</option>
              <option {{ old('certificate_type', $course->certificate_type)=='Ambos'   ?'selected':'' }}>Ambos</option>
            </select>
          </div>
        </div>
      </div>
    ...
    <div class="form-section">
      ...
      <div class="form-section-body">
          ...
          <div>
            ...
            <div class="file-upload-area">
              <div class="upload-wrap">
                <input type="file" name="image" accept="image/*" onchange="previewFile(this,'prev_image')">
                <div class="file-upload-icon">
                  <svg viewBox="0 0 24 24"><rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/><polyline points="21 15 16 10 5 21"/></svg>
                </div>
                <p class="file-upload-label">Cambiar portada</p>
            ...
            <div class="media-current">
              <div class="media-current-thumb-placeholder">
                <svg viewBox="0 0 24 24"><rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/><polyline points="21 15 16 10 5 21"/></svg>
              </div>
              <div>
                <p class="media-current-label">Imagen actual</p>
                <p class="media-current-name">{{ basename($course->image) }}</p>
              </div>
            </div>
            @endif
          </div>
          ...
          <div>
            ...
            <div class="file-upload-area">
              <div class="upload-wrap">
                ...
                <p class="file-upload-label">Cambiar banner</p>
            ...
            <div class="media-current">
              ...
              <div>
                <p class="media-current-label">Banner actual</p>
                <p class="media-current-name">{{ basename($course->banner) }}</p>
              </div>
            </div>
            @endif
          </div>
        ...
        <div>
          ...
          <input type="text" name="promo_video" class="f-control" placeholder="https://youtube.com/watch?v=..." value="{{ old('promo_video', $course->promo_video) }}">
        </div>
      </div>
    </div>
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  // Init price visibility based on current value
  togglePrice('{{ old('is_paid', $course->is_paid ? '1' : '0') }}');

  // Init teacher count badge
  updateTeacherCount();
});

function togglePrice(val) {
  document.getElementById('priceFields').style.display = val == 1 ? 'grid' : 'none';
}
